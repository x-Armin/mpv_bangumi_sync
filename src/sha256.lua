-- Public domain Lua SHA-256 implementation.
...
local function sha256(message)
  ...
  local function preprocess(msg)
    ...
    local zero_pad = 64 - ((len + 9) % 64)
    if zero_pad ~= 64 then
      msg = msg .. string.rep("\0", zero_pad)
    end

    return msg .. string.char(
      rshift(bit_len, 56) % 256,
      rshift(bit_len, 48) % 256,
      rshift(bit_len, 40) % 256,
      rshift(bit_len, 32) % 256,
      rshift(bit_len, 24) % 256,
      rshift(bit_len, 16) % 256,
      rshift(bit_len, 8) % 256,
      bit_len % 256
    )
  end

  local function process_chunk(chunk, hash)
    local w = {}
  ...
  for i = 1, #msg, 64 do
    hash = {process_chunk(msg:sub(i, i + 63), hash)}
  end
