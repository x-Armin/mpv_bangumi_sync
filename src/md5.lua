-- taken from https://github.com/Tony15246/uosc_danmaku/blob/main/modules/md5.lua
...
if _G.bit then
  ...
  bnot, bor, band, bxor = _G.bit.bnot, _G.bit.bor, _G.bit.band, _G.bit.bxor
  lshift, rshift = _G.bit.lshift, _G.bit.rshift
  rol, bswap = _G.bit.rol, _G.bit.bswap
elseif _G.bit32 then
  local bit32_bnot = _G.bit32.bnot
  tobit = function(a)
    return a <= 0x7fffffff and a or -(_G.bit32.bnot(a) + 1)
  end
  bnot = function(a)
    return tobit(bit32_bnot(tobit(a)))
  end
  bor, band, bxor = _G.bit32.bor, _G.bit32.band, _G.bit32.bxor
  lshift, rshift, rol = _G.bit32.lshift, _G.bit32.rshift, _G.bit32.lrotate
else
  return nil
end
...
local function md5_update(self, s)
  ...
  if len % 4 ~= 0 then
    m = m .. "\128" .. rep("\0", 63 - band(len + 8, 63))
      .. char(
        band(lshift(len, 3), 255),
        band(rshift(len, 5), 255),
        band(rshift(len, 13), 255),
        band(rshift(len, 21), 255)
      )
      .. "\0\0\0\0"
  end
